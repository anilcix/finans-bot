...
(function(){
  ...
  async function inject(){
    ...
    const raw=(data.points||[]).slice(-144);if(!raw.length)return;
    ...
    const card=document.createElement('div');card.id='flow15Card';card.className='card flow15-card';card.innerHTML=`
      <div class="flow15-head"><div><h2>Binance · 10dk Spot CVD + Perp OI</h2><div class="note">Son 24 saat · 144 × 10dk bucket</div></div><div class="flow15-badge">CVD ${spotSeries.length} · OI ${oiSeries.length} nokta</div></div>
      <div class="flow15-grid">
        <div class="flow15-panel"><div class="flow15-title">Binance Spot BTCUSDT CVD · Kümülatif USDT</div><div class="flow15-value">${moneyCompact(lastSpot.spot_cvd_cumulative_usd)}</div>${spotSeries.length>=2?'<canvas id="spotCvd15Canvas" class="flow15-canvas"></canvas>':`<div class="flow15-wait">${spotUnavailable?'Binance Spot API bu çalışma ortamından erişilemiyor. CVD için sahte/fallback veri çizilmiyor.':'Binance Spot CVD grafiği için en az 2 gerçek 10dk bucket bekleniyor.'}</div>`}<div class="flow15-note">Binance BTCUSDT spot aggTrades: agresif alış notionalı − agresif satış notionalı. Her nokta 10 dakikalık akışı temsil eder ve noktalar çizgiyle birleştirilir. Pozitif eğim spot alıcı baskısını, negatif eğim spot satıcı baskısını gösterir. Geçerli bucket: ${spotCoverage}/${rows.length}.${lastErr?` Son hata: ${lastErr}`:''}</div></div>
        <div class="flow15-panel"><div class="flow15-title">Binance BTC Perp OI · Coinalyze</div><div class="flow15-value">${moneyCompact(lastOi.binance_oi_usd)} ${oiChange==null?'':`<span style="font-size:11px;color:${oiChange>=0?'#39ff88':'#ff5c5c'}">${oiChange>=0?'+':''}${oiChange.toFixed(2)}%</span>`}</div>${oiSeries.length>=2?'<canvas id="oi15Canvas" class="flow15-canvas"></canvas>':'<div class="flow15-wait">Binance OI grafiği için en az 2 gerçek 10dk Coinalyze snapshot noktası bekleniyor.</div>'}<div class="flow15-note">Bu OI yalnızca Binance BTC perpetual verisidir ve Coinalyze üzerinden alınır. Her 10 dakikadaki OI snapshot noktaları çizgiyle birleştirilir.</div></div>
      </div>`;

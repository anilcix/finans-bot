...
(function(){
  ...
  const moneyCompact=v=>{if(!validNumber(v))return'—';v=Number(v);const a=Math.abs(v);const sign=v<0?'-':'';if(a>=1e9)return sign+'$'+(a/1e9).toFixed(2)+'B';if(a>=1e6)return sign+'$'+(a/1e6).toFixed(1)+'M';if(a>=1e3)return sign+'$'+(a/1e3).toFixed(1)+'K';return sign+'$'+a.toFixed(0)};
  const css=`
  ...
  function lineChart(canvas, rows, key, valueFormatter, zeroLine=false){
    if(!canvas)return;
    const series=rows.filter(r=>validNumber(r[key]));
    ...
    const vals=series.map(r=>Number(r[key]));
    let min=Math.min(...vals),max=Math.max(...vals);
    if(zeroLine){min=Math.min(min,0);max=Math.max(max,0)}
    if(min===max){const bump=Math.abs(min||1)*.01;min-=bump;max+=bump}
    else if(!zeroLine){const padY=(max-min)*0.12;min-=padY;max+=padY}
    const x=i=>pad.l+(series.length===1?pw/2:pw*i/(series.length-1));const y=v=>pad.t+ph*(1-(v-min)/(max-min));
    ...
    c.strokeStyle='#39ff88';c.lineWidth=2;c.beginPath();series.forEach((r,i)=>{const v=Number(r[key]),xx=x(i),yy=y(v);i?c.lineTo(xx,yy):c.moveTo(xx,yy)});c.stroke();
  ...
  async function inject(){
    ...
    const oiSeries=rows.filter(r=>validNumber(r.aggregate_oi_usd));
    const oiStart=oiSeries[0]||null,oiEnd=oiSeries[oiSeries.length-1]||null;
    let oiChange=null;if(oiStart&&oiEnd&&Number(oiStart.aggregate_oi_usd)!==0)oiChange=(Number(oiEnd.aggregate_oi_usd)/Number(oiStart.aggregate_oi_usd)-1)*100;
    const spotCoverage=raw.filter(r=>r.spot_ok&&validNumber(r.spot_cvd_delta_usd)).length;
    const spotSeries=rows.filter(r=>validNumber(r.spot_cvd_cumulative_usd));
    const lastSpot=spotSeries[spotSeries.length-1]||{};
    const lastOi=oiSeries[oiSeries.length-1]||{};
    const card=document.createElement('div');card.id='flow15Card';card.className='card flow15-card';card.innerHTML=`
      <div class="flow15-head"><div><h2>15dk Spot CVD + Perp OI</h2><div class="note">Son 24 saat · 96 × 15dk bucket</div></div><div class="flow15-badge">${rows.length}/96 bucket · OI ${oiSeries.length} nokta</div></div>
      <div class="flow15-grid">
        <div class="flow15-panel"><div class="flow15-title">Kraken Spot CVD · Kümülatif USD</div><div class="flow15-value">${moneyCompact(lastSpot.spot_cvd_cumulative_usd)}</div>${spotSeries.length>=2?'<canvas id="spotCvd15Canvas" class="flow15-canvas"></canvas>':'<div class="flow15-wait">CVD grafiği için en az 2 gerçek 15dk bucket bekleniyor.</div>'}<div class="flow15-note">Her 15 dakikada Kraken BTC/USD spot agresif alış notionalı eksi agresif satış notionalı hesaplanır; grafik 24 saat içindeki kümülatif akışı gösterir. Pozitif eğim spot alıcı baskısı, negatif eğim spot satıcı baskısıdır. Geçerli bucket: ${spotCoverage}/${rows.length}.</div></div>
        <div class="flow15-panel"><div class="flow15-title">Coinalyze Aggregate BTC Perp OI</div><div class="flow15-value">${moneyCompact(lastOi.aggregate_oi_usd)} ${oiChange==null?'':`<span style="font-size:11px;color:${oiChange>=0?'#39ff88':'#ff5c5c'}">${oiChange>=0?'+':''}${oiChange.toFixed(2)}%</span>`}</div>${oiSeries.length>=2?'<canvas id="oi15Canvas" class="flow15-canvas"></canvas>':'<div class="flow15-wait">OI grafiği için en az 2 gerçek Coinalyze snapshot noktası bekleniyor. Null/veri olmayan bucket 0 olarak çizilmiyor.</div>'}<div class="flow15-note">OI spot değildir; bu panel Coinalyze üzerinden seçili büyük BTC perpetual piyasalarının aggregate açık pozisyonunu her 15 dakikada snapshot olarak kaydeder. CVD ile birlikte okununca hareketin spot talep mi yoksa kaldıraç birikimi mi olduğunu ayırmaya yardımcı olur.</div></div>
      </div>`;
    content.insertBefore(card,content.firstChild);
    const draw=()=>{if(spotSeries.length>=2)lineChart(document.getElementById('spotCvd15Canvas'),rows,'spot_cvd_cumulative_usd',moneyCompact,true);if(oiSeries.length>=2)lineChart(document.getElementById('oi15Canvas'),rows,'aggregate_oi_usd',moneyCompact,false)};

...
def _chain(currency="BTC"):
    ...
    for x in get_book_summary(currency):
        try:
            ...
            continue
        ex.setdefault(expiry, []).append(
            (strike, is_call, x.get("open_interest", 0) or 0, x.get("mark_iv", 0) or 0)
        )
    if not ex:
...
def _funding_stats(currency="BTC"):
    ...
    try:
        ...
        rows = _okx(
            ...
            {"instId": f"{currency}-USDT-SWAP", "limit": 21},
        )
    ...
    return {
        "avg_pct": avg * 100,
        "annualized_pct": avg * 3 * 365 * 100,
        "positive_pct": sum(1 for x in rates if x > 0) / len(rates) * 100,
        "source": source,
        "samples": len(rates),
    }
...
def _positioning(currency="BTC"):
    ...
    try:
        ...
        return {
            "top_account_long": top_accounts,
            "top_position_long": top_positions,
            "global_long": global_long,
            "whale_retail_gap": top_accounts - global_long,
            "money_vs_heads_gap": top_positions - top_accounts,
            "source": "Binance",
            "mode": "top_traders",
        }
    except Exception:
        ...
        row = rows[0]
        return {
            "global_long": float(row["buyRatio"]) * 100,
            "global_short": float(row["sellRatio"]) * 100,
            "source": "Bybit",
            "mode": "market_account_ratio",
        }
...
def _oi_metrics(currency="BTC"):
    ...
    try:
        ...
        r.raise_for_status()
        rows = r.json()
        vals = [float(x["sumOpenInterest"]) for x in rows]
        mean = sum(vals) / len(vals)
        std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5
        return {
            "zscore": (vals[-1] - mean) / std if std else 0,
            "open_interest_btc": vals[-1],
            "source": "Binance",
        }
    except Exception:
        pass
    ...
    try:
        ...
        vals = [float(x["openInterest"]) for x in ordered]
        if len(vals) >= 5:
            mean = sum(vals) / len(vals)
            std = (sum((v - mean) ** 2 for v in vals) / len(vals)) ** 0.5
            return {
    ...
    row = _okx(
        ...
        {"instType": "SWAP", "instId": f"{currency}-USDT-SWAP"},
    )[0]
    return {
        "zscore": None,
        "open_interest_btc": float(row["oiCcy"]),
        "source": "OKX",
    }
...
def get_analysis_data(currency="BTC"):
    ...
    try:
        ...
        r.raise_for_status()
        d = r.json()["data"][0]
        data["fear_greed"] = {"value": int(d["value"]), "label": d["value_classification"]}
    except Exception:
        pass
    ...
    try:
        ...
        data["max_pain"] = {
            ...
            "put_call_ratio": puts / calls if calls else 0,
        }
    except Exception:
        pass
    ...
    try:
        ...
        for x in summary:
            ...
            if oi and iv and t > 0:
                g += (1 if is_call else -1) * bs_gamma(spot, strike, t, iv / 100) * oi * (spot ** 2) * 0.01
        data["gex_musd"] = g / 1e6
    except Exception:
        pass
